...
# ======================================================================================================================
# S-60 - APPROXIMATE RESISTANCE AND POWER PREDICTION METHOD FOR CRUISING SPEED
# ======================================================================================================================

LengthWLft = LengthWL * 3.28084

x1 = 1
x2 = 2 * (LengthPP / Beam - 7) / 3
x3 = 2 * (Beam / Draught - 3)
x4 = 10 * (CB - 0.7)
x5 = (LCB - 0.515) / 2.995
x6 = x2**2
...
#f = open('ResCoef\\S60_Coef\\S60ResistanceRegCoef.csv', 'r')
f = open('S60ResistanceRegCoef.csv', 'r')
a050 = list()
a055 = list()
a060 = list()
a065 = list()
...
for line in lines:
    ...
    a085.append(float(line[7]))
    a090.append(float(line[8]))
f.close()
...
while (i < l):
    ...
    elif 0.80 <= V_L_Cruising < 0.85:
        a.append(a080[i] + (V_L_Cruising - 0.80) * (a085[i] - a080[i]) / (0.85 - 0.80))
    elif 0.85 <= V_L_Cruising < 0.90:
        a.append(a085[i] + (V_L_Cruising - 0.85) * (a090[i] - a085[i]) / (0.90 - 0.85))
    elif 0.90 <= V_L_Cruising:
        a.append(a085[i] + (V_L_Cruising - 0.85) * (a090[i] - a085[i]) / (0.90 - 0.85))
    i += 1
...
CR400Cruising = 8.3375 * Y400Cruising + 17.3505
...
#f = open('ResCoef\\S60_Coef\\S60CorrectiveCoef.csv', 'r')
f = open('S60CorrectiveCoef.csv', 'r')
k06 = list()
k08 = list()
k10 = list()
k12 = list()
...
while n < p:
    ...
    while (i < l):
        ...
        elif 0.80 <= V_L[n] < 0.85:
            a.append(a080[i] + (V_L[n] - 0.80) * (a085[i] - a080[i]) / (0.85 - 0.80))
        elif 0.85 <= V_L[n] < 0.90:
            a.append(a085[i] + (V_L[n] - 0.85) * (a090[i] - a085[i]) / (0.90 - 0.85))
        elif 0.90 <= V_L[n]:
            a.append(a085[i] + (V_L[n] - 0.85) * (a090[i] - a085[i]) / (0.90 - 0.85))
        i += 1
    ...
    TotalResS60iter = 5.045 * 10**(-3) * ((Displacement * (SpeedMS[n])**2) / LengthPP) * CR

    TotalResS60.append(TotalResS60iter)
